...
def fetch_basedt_minus3dt_minus20dt():
    ...
    # 逆序取数
    sql_ZN_Futures = 'select * from click_CFDc  order by id desc ; '
    # 取第一行数
    ...
    _base_time_minus50 = minum_basetime_minus_N_minutes(base_time_string,15)
    _base_time_minus60 = minum_basetime_minus_N_minutes(base_time_string,20)
    base_dt_minus3_dt = difference_set_fromTwo_df(df_ZN_Futures,"LastTime",_base_time_minus4,_base_time_minus3)
    base_dt_minus20_dt = difference_set_fromTwo_df(df_ZN_Futures,"LastTime",_base_time_minus60,_base_time_minus50)
...
if __name__=="__main__":
    ...
    remove_file("txt")
    time.sleep(2)
    trade_base_params_dict = {'DowJ': 410}
    while True:
        e = datetime.datetime.now()
        print(e)
        base_dt,base_dt_minus3_dt,base_dt_minus20_dt = fetch_basedt_minus3dt_minus20dt()

        find_and_confirm_signal("ym",float(base_dt[0]),float(base_dt_minus3_dt[0]),float(base_dt_minus20_dt[0]),trade_base_params_dict,0.33,0.13)
